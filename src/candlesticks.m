function [varargout] = candlesticks(Open,High,Low,Close, varargin)
    ...
    otherwise
        if nargin > 5
            ...
            delta = 4-nargin;
            errorMsg = 'Missing %d argument';
            if delta > 1
                errorMsg = strcat(errorMsg, 's');
            end
            errorMsg = strcat(errorMsg, '!');
            error(errorMsg, 4-nargin);
        end
    end
    ...
    bearCandlesticks = 40;
    bullCandlesticks = 40;
    neutralCandlesticks = 10;
    bear    = cell(bearCandlesticks,1);
    bull    = cell(bullCandlesticks,1);
    neutral = cell(neutralCandlesticks,1);
    ...
    case "map"
        ...
        varargout{3} = neutral;
    case "signals"
        varargout{1} = signalizeOccurrences(bullCandlesticks, observ, bull);
        varargout{2} = signalizeOccurrences(bearCandlesticks, observ, bear);
        varargout{3} = signalizeOccurrences(neutralCandlesticks, observ, neutral);    
    end
    
    function signals = signalizeOccurrences(nCandlesticks, observ, map)
        signals = zeros(nCandlesticks, observ);
        for i = 1: nCandlesticks
            signals(i, map{i}) = 1;
        end    
    end
